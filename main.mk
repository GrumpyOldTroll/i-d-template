...
%.xml: %.md *.yang *.json
	...
	echo "vals: $<" ; \
	if [ "$$h" = '---' ]; then \
		if bash -x -c "grep -q YANG-MODULE $<" ; then \
			$(LIBDIR)/yang-inject.sh $< ; \
			echo '$(subst ','"'"',cat $<.withyang $(MD_PREPROCESSOR) $(REMOVE_LATEST) | $(kramdown-rfc2629) > $@)'; \
			cat $<.withyang $(MD_PREPROCESSOR) $(REMOVE_LATEST) | $(kramdown-rfc2629) > $@; \
		else \
